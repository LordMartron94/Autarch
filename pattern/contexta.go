...
/*
LookaheadConstraint is one conjunct of a production guard: Peek(Offset).Token must equal Token.

Offset uses the same convention as Syntaxa’s SelectRuleContext.Peek (0 = current token in
the visible stream after skip). An empty Guard slice means the production is unconstrained.
Multiple entries are ANDed.

Time complexity: O(1)
Space complexity: O(1)
*/
type LookaheadConstraint[TTokenID comparable] struct {
	Offset int
	Token  TTokenID
}
...
Guard, when non-empty, restricts when this production is considered for disambiguation
(choice dispatch, guarded FIRST). It does not change the language generated by the CFG.
